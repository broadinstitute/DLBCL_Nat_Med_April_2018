...
#
#
writeMirnaReport <- function(expdata, kclus, bestclu, allcluster, markers, cormatrix, markersP, heatmap, heatmapall, images){
    ...
    #Start HTML   
    fhReport <- newReport("Clustering of miR expression: consensus NMF")
...
method1 <- addTo( newSubSection( "CNMF Method" ),
				...
				that has been shown to identify molecular patterns when applied to miR expression data ", asReference( fullCitation ),",", asReference( webCitation ),".
				Rather than separating miR clusters based on distance computation, NMF detects contextdependent
				patterns of miR expression in complex biological systems." ) );
...
fhReport <- addToInput( fhReport, 
				newParagraph( asStrong( "miR Array Platforms " ),
					"Agilent H-miR_8x15K and H-miR_8x15Kv2." ),
				newParameterList( asParameter( "miR expression file" ),
					asFilename( expdata ) ) );				
