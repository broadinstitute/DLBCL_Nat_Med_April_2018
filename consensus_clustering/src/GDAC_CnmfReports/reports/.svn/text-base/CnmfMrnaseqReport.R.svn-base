...
#
#
writemRNAseqReport <- function(expdata,kclus,bestclu,allcluster,markers,cormatrix,markersP,heatmap,heatmapall, images){
	...
	# Start a new report
    fhReport <- newReport("Clustering of mRNAseq gene expression: consensus NMF")
	...
	fhReport <- addToSummary(fhReport,
		newParagraph("The most robust consensus NMF clustering of ", asParameter(samplenum), 
			" samples using the ", asParameter(genenumber), " most variable genes
