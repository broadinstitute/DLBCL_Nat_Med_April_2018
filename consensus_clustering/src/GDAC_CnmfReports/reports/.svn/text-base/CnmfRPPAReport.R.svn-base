...
#
#
writenmfRPPAReport <- function(expdata,kclus,bestclu,allcluster,markers,cormatrix,markersP,heatmap,heatmapall, images){
	...
	fullCitation <- newCitation(
		...
		title="Metaproteins and molecular pattern discovery using matrix factorization", 
		publication="Proc Natl Acad Sci U S A", issue="12", number="101", pages="4164-9", year="2004", 
		url="http://www.ncbi.nlm.nih.gov/pubmed?term=Metaproteins%20and%20molecular%20pattern%20discovery%20using%20matrix%20factorization" );
	...
	webCitation <- newCitation( title="Broad proteinpattern: NMFConsensus", url="http://proteinpattern.broadinstitute.org/gp/pages/index.jsf" );
	...
	# Start a new report
    fhReport <- newReport("Clustering of RPPA data: consensus NMF")
	...
	fhReport <- addToSummary(fhReport,
		newParagraph("The most robust consensus NMF clustering of ", asParameter(samplenum), 
			" samples using ", asParameter(proteinnumber), " proteins
...
# Removed as the figure is currently flawed
	...
	figheatmapall <- newFigure(basename(heatmapall), fileHighRes = basename(heatmapall),
		...
		asParameter(samplenum), " samples and the ", asParameter(proteinnumber), " proteins.");
...
# Removed as the figure is currently flawed
	...
	cnmfMethod <- addTo(
		...
		newParagraph("Non-negative matrix factorization (NMF) is an unsupervised 
			...
			when applied to protein expression data ", 
			asReference(fullCitation), ",", asReference(webCitation), 
			". Rather than separating protein clusters based on distance 
			computation, NMF detects contextdependent patterns of protein expression
