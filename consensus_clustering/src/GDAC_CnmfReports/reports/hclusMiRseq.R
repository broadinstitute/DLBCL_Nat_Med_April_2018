...
writehclumiRseqReport <- function(expdata, kclus, bestclu, allcluster, markers, cormatrix, markersP, heatmap, heatmapall, images){
    ...
    #Start HTML   
    fhReport <- newReport("Clustering of miRseq precursor expression: consensus hierarchical")
...
Input <- addTo(
				newParagraph("miRseq (at precursor level) of RPM value (reads per million reads aligned to miRBase precursor) with log2 transformed was as the input data for the clustering"));
